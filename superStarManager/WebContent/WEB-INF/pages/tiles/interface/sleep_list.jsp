...
function batchImport(){
	$('#winlist').dialog('open').dialog('setTitle','批量上传界面');
}
	...
	if(result){
		...
		if($('#interfaceSort').val()!=""){
			...
			$.ajax({
				...
				type : 'post',
				data : {
					'itf.interface_type':'2', // 只查询屏保界面数据 过滤条件
...
function validateFormList(){
	var result = $("#saveInterfaceList").validate({  
    	rules: {
        	"itf.interface_time": {
				required: true,
				digits:true,
	...
	if(result){
		if($('#b_onlineTime').datebox('getValue')==""){
			 $.messager.alert('提示','上线时间不能为空', 'info');
			 return false ; 
		}
		if($('#path').val()==""){
		...
		//判断时间
		if($('#b_onlineTime').datebox('getValue')!="" && $('#b_offlineTime').datebox('getValue')!=""){
			var startTime=$("#b_onlineTime").datebox('getValue');  
		    var start=new Date(startTime.replace("-", "/").replace("-", "/"));  
		    var endTime=$("#b_offlineTime").datebox('getValue');  
		    var end=new Date(endTime.replace("-", "/").replace("-", "/"));  
		    if(end<start){  
		    	$.messager.alert('提示','上线时间不能大与下线时间', 'info');
		        return false;  
...
function saveInterface(){
		...
		$('#saveInterface').form('submit', {
			...
			success : function(data) {
				...
				if (data.success) {
					...
					closeWin();
					$('#tt').datagrid({url:'sleepInterfaceList.action',queryParams:"",pageNumber:1});
				}else {
   ...
   function saveInterfaceList(){
	    var path=$('#path').val();
		$('#saveInterfaceList').form('submit', {
		    url:'saveSleepInterfaceList.action',
		    data : {
			  'path' : path
		    }, 
		    onSubmit: function(){
	...
	function removeInterface() {
		...
		if (rows.length == 0) {
			...
			$.messager.confirm('删除记录', '确定要删除选中的记录吗?', function(result) {
				...
				$.ajax({
					url : 'removeSleepInterface.action',
					data : {
						'ids' : ids
					},
					type : 'post',
	...
	function searchInterface() {
		...
		$('#tt').datagrid({
			url : 'searchSleepInterface.action',
...
<div id="toolbar">
	...
	<a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="batchImport()">批量上传</a>
</div>
	...
	<form id="saveInterface" method="post" enctype="multipart/form-data">
		<table>
			...
			<tr>
				...
				<td><input type="text" class="easyui-datebox" id="onlineTime" name="itf.online_time" /></td>
				<td>下线时间</td>
				<td><input type="text" class="easyui-datebox" id="offlineTime" name="itf.offline_time" /></td>
			</tr>
			<tr>
				<td>
				...
				<td>
					<input type="text" id="interfaceTime" name="itf.interface_time" onchange="checkNumber(this)"/>
				</td>
				<td>
...
<div id="winlist" class="easyui-dialog" style="padding:5px;width:500px;height:400px;"
			title="My Dialog" iconCls="icon-ok"
			closed="true"
			modal="true"
			buttons="#batch-buttons"
			>
	<form id="saveInterfaceList" method="post" enctype="multipart/form-data">
		<table>
			<tr>
				<td>上线时间<font color="red">*</font></td>
				<td><input type="text" class="easyui-datebox" id="b_onlineTime" name="itf.online_time" /></td>
				<td>下线时间</td>
				<td><input type="text" class="easyui-datebox" id="b_offlineTime" name="itf.offline_time" /></td>
			</tr>
			<tr>
				<td>
					播放时长<font color="red">*</font>
				...
				<td>
					<input type="text" id="b_interfaceTime" name="itf.interface_time" onchange="checkNumber(this)"/>
				</td>
			</tr>
			<tr>
                 <td><input type=button value="批量上传" onclick="browseFolder('path')"/></td>
              
                <td><input id="path" type="text" name="path" size="20"/> 
                </td>
...
<div id="batch-buttons" >
		<table cellpadding="0" cellspacing="0" style="width:100%">
			<tr>
				<td style="text-align:right">
					<a href="#" class="easyui-linkbutton" iconCls="icon-save" onclick="saveInterfaceList()">保存</a>
					<a href="#" class="easyui-linkbutton" iconCls="icon-cancel" onclick="closeBatchWin()">关闭</a>
				</td>
			</tr>
		</table>
</div>
<div id="searchDialog" class="easyui-dialog" style="padding:5px;width:500px;height:300px;"
		...
		<form id="searchForm" method="post">
			<table>
				...
				<tr>
					<td>上线时间</td>
					<td><input type="text" class="easyui-datebox"  id="s_onlineTime_start" name="itf.online_time" /></td>
					<td>至</td>
					<td><input type="text" class="easyui-datebox"  id="s_onlineTime_end" name="itf.offline_time" /></td>
				</tr>
				<tr>
					<td>下线时间</td>
					<td><input type="text" class="easyui-datebox"  id="s_offlineTime_start" name="itf.online_time" /></td>
					<td>至</td>
					<td><input type="text" class="easyui-datebox"  id="s_offlineTime_end" name="itf.offline_time" /></td>
				</tr>
				<tr>
					<td>
						播放时长
