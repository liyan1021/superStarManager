...
$(function(){
	$('#tt').datagrid({
		...
		columns:[[
			...
			{field:'music_id',title:'歌曲 ID',width:60,hidden:true},
			{field:'music_name',title:'歌曲名字',width:150},
...
function editAdvert(index){
    ...
    var row = $('#tt').datagrid('getSelected');  
   	if (row){  
        ...
        $('#musicId').val(row.music_id);
        $('#musicName').val(row.music_name);
...
function validateForm(){
	...
	if(result){
		...
		if($('#advertSort').val()!=""){
			...
			$.ajax({
				async: false,
				url:'checkAdvertVideoSort1.action',
				type : 'post',
				data : {
...
function saveAdvert(){
	...
	function searchAdvert() {
		var gridparam = {
			...
			"advertVideo.music_name" : $('#s_musicName').val()
			/* "advertVideo.is_activity" : $('#s_isActivity').combobox('getValue'), */
		};
		$('#tt').datagrid({
			url : 'searchAdvertVideo.action',
	...
	function fitMusic(){
		...
		if (rows.length == 0) {
			...
			$('#musicId').val(rows[0].song_id);
			$('#musicName').val(rows[0].song_name);
	...
	function searchMusic(){
		var gridparam = {
				"song1.song_name" : $('#s_music_songName').val(),
				"song1.singer_name" : $('#s_music_singerName').val()
			};
			$('#musicGrid').datagrid({
				url : 'unAdvertVideoList.action',
				queryParams : gridparam
		...
		<table>
			<tr>
				...
				<td>
	        		...
	        		<input type="text"  id="advertTheme" name="advertVideo.advert_theme"/>
	        	</td>
			...
			<tr>
				...
				<td><input type="text" class="easyui-datebox" id="onlineTime" name="advertVideo.online_time" data-options="editable:false"/></td>
				<td>下线时间</td>
				<td><input type="text" class="easyui-datebox" id="offlineTime" name="advertVideo.offline_time" data-options="editable:false"/></td>
			</tr>
			<tr>
				<td>歌曲名字 <font color="red">*</font></td>
				<td>
					<input type="hidden" id="musicId" name="advertVideo.music_id"  />
	        		<input type="text"  id="musicName" name="advertVideo.music_name"  readonly="readonly"/>
	        	</td>
	        	<td>
		...
		<form id="searchForm" method="post">
			<table>
				...
				<tr>
					...
					<td>
						<input type="text" id="s_musicName" name="advert.music_name" />
					</td>
					<%-- <td>是否可用</td>
					<td>
