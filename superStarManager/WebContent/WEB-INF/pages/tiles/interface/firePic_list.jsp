...
function loadPlace(){
           $.ajax({ 
				url : 'firePicInterfaceList.action',
				type : 'post',
...
function saveInterface(){
	if(!$('#interfaceId').val()){
		...
		$('#saveInterface').form('submit', {
		    url:'saveFirePicInterface.action',
		    onSubmit: function(){
		       //校验 return flase ;
		    	return validateForm();
			},
			...
			//修改
			$('#saveInterface').form('submit', {
				url : 'editFirePicInterface.action',
				onSubmit : function() {
					return validateForm();
				},
				success : function(data) {
	...
	function pushInterface(){
		 ...
		 if(state=="已发布"){
			 $.messager.alert("提示",'已发布的不能再次发布','info');
              ...
              ids=$('#interfaceId').val();
				$.ajax({
					url : 'pushFirePicInterface.action',
					data : {
						'ids' : ids
					},
					type : 'post',
...
<div class="easyui-panel" title="消防图解界面设置">
